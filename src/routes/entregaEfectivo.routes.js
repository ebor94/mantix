...
router.get('/:id/comprobante-pdf', auth, controller.comprobantePdf);

// Registrar entrega (cajero/admin).
router.post('/', auth, requirePermiso('caja', 'ver_cuadre'), controller.crear);

// Confirmar con OTP (cajero/admin).
router.post('/:id/confirmar', auth, requirePermiso('caja', 'ver_cuadre'), controller.confirmar);
