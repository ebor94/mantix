...
async function crearReciboParaAfiliacion(afiliado, transaction) {
  ...
  if (!Number.isFinite(valorRecibido) || valorRecibido < 5000) {
    throw new AppError('El valor recibido debe ser mayor o igual a $5.000 para emitir el recibo de caja.', 400);
  }
...
const INCLUDE_RECIBO_COMPLETO = [
  {
    model: Afiliado,
    as: 'afiliado',
    attributes: ['id', 'numeroDocumento', 'numeroContrato', 'primerNombre', 'segundoNombre', 'primerApellido', 'segundoApellido', 'celular', 'email', 'direccion', 'barrio', 'ciudad', 'departamento', 'estadoRegistro', 'rechazado', 'rechazadoParcial', 'anulado', 'fechaPago']
  },
