  ...
  <MainLayout>
    <div class="space-y-6">
      ...
      <div class="flex items-center justify-between">
        ...
        <!-- ✅ Solo mostrar botones de vista cuando NO estamos en tab consulta -->
        <div v-if="activeTab !== 'consulta'" class="flex items-center space-x-3">
       
        </div>
      </div>

      <!-- ✅ Filtros: Solo mostrar cuando NO estamos en tab consulta -->
      <div v-if="activeTab !== 'consulta'" class="card">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
          <!-- Búsqueda -->
          <div class="md:col-span-2">
            <label class="block text-sm font-medium text-gray-700 mb-2">Buscar</label>
            <div class="relative">
              <input v-model="filters.search" type="text" placeholder="Buscar por nombre o sede..."
      ...
      <!-- ✅ NUEVO: Contenido del tab "Consulta por Fechas" -->
      <div v-if="activeTab === 'consulta'">
        <MantenimientosFiltroFechas />
      </div>

      <!-- ✅ Contenido de los otros tabs (Todos, Hoy, Próximos, Atrasados) -->
      <div v-else>
        <!-- Vista de Lista -->
        <div v-if="viewMode === 'list'">
          ...
          <div v-else-if="currentMantenimientos.length === 0" class="text-center py-12">
            <EmptyState title="No hay mantenimientos"
              description="No se encontraron mantenimientos con los filtros aplicados" icon="calendar" />
          </div>

          <div v-else class="space-y-4">
            <MantenimientoCard v-for="mantenimiento in currentMantenimientos" :key="mantenimiento.id"
              :mantenimiento="mantenimiento" @ejecutar="openEjecutarModal" @reprogramar="openReprogramarModal"
    ...
    <!-- Modal de Ejecución -->
    <EjecutarMantenimientoModal v-if="showEjecutarModal" :mantenimiento="selectedMantenimiento"
      @close="showEjecutarModal = false" @success="handleEjecutado" />

    <!-- Modal de Reprogramación -->
    <ReprogramarMantenimientoModal v-if="showReprogramarModal" :mantenimiento="selectedMantenimiento"
      @close="showReprogramarModal = false" @success="handleReprogramado" />
...
// ✅ Agregar categoria_id al objeto de filtros
const filters = ref({
  search: '',
  sede_id: null,
  estado_id: null,
...
const tabs = computed(() => [
  ...
  { label: 'Consultar por Fechas', value: 'consulta', icon: 'search' }
])

// ✅ Actualizar currentMantenimientos para incluir filtro por categoría
const currentMantenimientos = computed(() => {
  ...
  if (filters.value.search) {
    ...
    data = data.filter(m =>
      ...
      m.actividad?.equipo?.nombre?.toLowerCase().includes(search)
    )
  }
  ...
  if (filters.value.categoria_id) {
    data = data.filter(m => m.actividad?.categoria_id === filters.value.categoria_id)
  }
...
// ✅ Actualizar clearFilters para incluir categoria_id
const clearFilters = () => {
  filters.value = {
    search: '',
    sede_id: null,
...
// ✅ Actualizar loadCatalogos para incluir categorías
const loadCatalogos = async () => {
  try {
    const [sedesRes, estadosRes, categoriasRes] = await Promise.all([
      api.get('/sedes'),
      api.get('/estados?tipo=mantenimiento'),
      api.get('/categorias-mantenimiento')  // ✅ NUEVO
    ])

    sedes.value = sedesRes
    console.log('Sedes cargadas:', sedesRes)

    estados.value = estadosRes.data || estadosRes
    console.log('Estados cargados:', estados.value)

    categorias.value = categoriasRes.data  // ✅ NUEVO
